...
main()
{
	...
	spr_make(0,180,121,0x5000,FLIP_X_MASK|SIZE_MAS,FLIP_X|SZ_32x64,0,1);
	set_sprpal(0,lapinpal,1);
	load_vram(0x5000,lapin,0xA00);

	spr_make(1,52,121,0x6200,FLIP_MAS|SIZE_MAS,NO_FLIP|SZ_32x64,1,1);
	set_sprpal(1,alicepal,1);
	load_vram(0x6000,alice,0xA00);	

	spr_make(2,200,100,0x5A00,FLIP_MAS|SIZE_MAS,NO_FLIP|SZ_16x16,2,1);
	set_sprpal(2,ballpal,1);
	load_vram(0x5A00,ball,0x100);
...
*/
	...
	spr_set(0);
	if ( state_lapin == 0) player_lapin_0();
	if ( state_lapin == 1) player_lapin_1();
	if ( state_lapin == 2) player_lapin_2();
	if ( state_lapin == 3) player_lapin_3();
	if ( state_lapin == 4) player_lapin_4();
	if ( state_lapin == 5) player_lapin_5();
	if ( state_lapin == 6) player_lapin_6();
...
player_lapin_0()
{
	if ( tics_lapin == 0 ) spr_pattern(0x5200);
	tics_lapin = 1;
	if (j2 & JOY_LEFT)
	{
		/* spr_ctrl(FLIP_X_MASK,FLIP_X); */
...
player_lapin_1()
{
	...
	if (tics_lapin > 6)
	{
		tics_lapin = 0;
		frame_lapin++;
		if (frame_lapin > 3) frame_lapin = 0;
		if (frame_lapin == 0) spr_pattern(0x5200);
		if (frame_lapin == 1) spr_pattern(0x5000);
		if (frame_lapin == 2) spr_pattern(0x5200);
		if (frame_lapin == 3) spr_pattern(0x5400);
	}
	if (j2 & JOY_LEFT && dir_lapin == DIR_LEFT)
	{
		walk_lapin = 1;
	}
	if (j2 & JOY_RGHT && dir_lapin == DIR_RIGHT)
	{
		walk_lapin = 1;
	}
	if (j2 & JOY_A)
...
{
	play_state = 1;
	if (tics_lapin == 0) spr_pattern(0x5800);
	if (tics_lapin == 4) spr_pattern(0x5600);
	tics_lapin++;
	if (tics_lapin > 8)
	{
		tics_lapin = 0;
...
physique()
{
	...
	if (lapin_x < 128) lapin_x = 128;
	if (lapin_x > 232) lapin_x = 232;
	lapin_y = lapin_y - lapin_v;
	if (lapin_y < 121) lapin_v--;
	if (lapin_y > 121) lapin_y = 121;
	spr_put(0,lapin_x,lapin_y, dir_lapin);
...
score()
{
	...
	if(point_alice == max | point_lapin == max)
	{
		point_alice = 0;
		point_lapin = 0;
		play_state = 0;
